...
def test_get_session_yields_session_and_closes() -> None:
    ...
    async def consume() -> AsyncSession:
        """get_session を正常系で最後まで消費する関数.

        Returns:
            AsyncSession: get_session が yield したセッション.
        """
    ...
    with patch.object(engine_module, "async_session", MagicMock(return_value=session)):
...
def test_get_session_rolls_back_and_reraises_on_exception(exc_type: type[BaseException]) -> None:
    ...
    async def consume_with_error() -> None:
        """get_session の yield 中に例外を送出する関数."""
        gen = engine_module.get_session()
        await anext(gen)
        await gen.athrow(exc_type("boom"))

    with (
        patch.object(engine_module, "async_session", MagicMock(return_value=session)),
